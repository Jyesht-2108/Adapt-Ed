...
import { Mic, MicOff, Volume2, VolumeX, Loader2, CheckCircle2, XCircle, RotateCcw, Trophy, AlertCircle, Eye, ShieldAlert } from 'lucide-react';
...
export function VivaRoom({ sessionId, moduleTitle, userGoal, onComplete, onRetake }: VivaRoomProps) {
  ...
  const handleViolation = (type: string, message: string) => {
    if (isTerminated || isComplete) return; // Don't log violations after termination
  ...
  useEffect(() => {
    if (strikes >= 3 && !isTerminated && !isComplete) {
      ...
      // Call backend to mark as dishonesty
      terminateExam();
    }
  }, [strikes, isTerminated, isComplete]);
  
  // Terminate exam due to academic dishonesty
  const terminateExam = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/viva/complete`, {
        ...
        body: JSON.stringify({
          user_id: userId,
          module_id: moduleTitle,
          final_score: 0, // Will be forced to 0 by backend anyway
          academic_dishonesty_flag: true,
      ...
      toast({
        title: 'Error',
        description: 'Failed to submit exam termination. Please contact support.',
        variant: 'destructive',
      });
    }
  };
  ...
  // Get strike badge variant
  const getStrikeBadgeVariant = () => {
    if (strikes === 0) return 'default';
    if (strikes <= 2) return 'secondary';
    return 'destructive';
  };
  
  // Get strike badge color
  ...
  if (!isComplete) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 p-4">
        {/* ProctorEye Component */}
        <ProctorEye 
          onViolation={handleViolation}
          isActive={!isTerminated && !isComplete}
        />
              ...
              <DialogDescription className="space-y-4 pt-4">
                <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4">
                  ...
                  <p className="text-red-800 text-sm">
                    Your examination has been terminated due to multiple integrity violations.
                  </p>
                </div>
              ...
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    <Button
                      size="lg"
                      disabled={isProcessing || isSpeaking || !speechSupported || isTerminated}
